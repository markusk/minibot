...
#            rospy.logerr('unable to initialize IMU at port {}'.format(serial_port))
    ...
    def publish_data(self):
        ...
        """
        # debug lines
        rospy.loginfo('IMU x=%s', self.imu_msg.orientation.x)
        rospy.loginfo('IMU y=%s', self.imu_msg.orientation.y)
        rospy.loginfo('IMU z=%s', self.imu_msg.orientation.z)
        rospy.loginfo('IMU w=%s', self.imu_msg.orientation.w)
        # sleep 0.5 seconds
        rospy.sleep(0.5)
        """
